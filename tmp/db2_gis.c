 ...
 *
 * oracle_gis.c
 * 		routines that convert between DB2 SDO_GEOMETRY and PostGIS EWKB
 *
 *-------------------------------------------------------------------------
 */
...
#define uintToNumber(errhp, intp, numberp) { \
	if (checkerr( \
			...
			(errhp)) != OCI_SUCCESS) \
		db2Error_d(FDW_ERROR, "OCINumberFromInt failed to convert integer to NUMBER", oraMessage); \
}
...
#define numberToUint(errhp, numberp, intp) { \
	if (checkerr( \
			...
			(errhp)) != OCI_SUCCESS) \
		db2Error_d(FDW_ERROR, "OCINumberToInt failed to convert NUMBER to integer", oraMessage); \
}
...
#define doubleToNumber(errhp, doublep, numberp) { \
	if (checkerr( \
			...
			(errhp)) != OCI_SUCCESS) \
		db2Error_d(FDW_ERROR, "OCINumberFromReal failed to convert floating point number to NUMBER", oraMessage); \
}
...
#define numberToDouble(errhp, numberp, doublep) { \
	if (checkerr( \
			...
			(errhp)) != OCI_SUCCESS) \
		db2Error_d(FDW_ERROR, "OCINumberToReal failed to convert NUMBER to floating point number", oraMessage); \
}
...
/* maps DB2 SRIDs to PostGIS SRIDs */
...
/*
 * Structures needed for managing the MDSYS.SDO_GEOMETRY DB2 type.
...
static unsigned ewkbType(db2Session *session, ora_geometry *geom);
static unsigned sdoDimension(db2Session *session, ora_geometry *geom);
static unsigned ewkbIsMeasured(db2Session *session, ora_geometry *geom);
static unsigned ewkbSrid(db2Session *session, ora_geometry *geom);
static unsigned numCoord(db2Session *session, ora_geometry *geom);
static double coord(db2Session *session, ora_geometry *geom, unsigned i);
static unsigned numElemInfo(db2Session *session, ora_geometry *geom);
static unsigned elemInfo(db2Session *session, ora_geometry *geom, unsigned i);
static void appendElemInfo(db2Session *session, ora_geometry *geom, unsigned info );
static void appendCoord(db2Session *session, ora_geometry *geom, double coord);
...
static void unpack(db2Session *session, ora_geometry *geom);
static void freeUnpacked(db2Session *session, ora_geometry *geom);

/* All ...Fill() functions return a pointer to the end of the written zone
 */
static unsigned ewkbHeaderLen(db2Session *session, ora_geometry *geom);
static char *ewkbHeaderFill(db2Session *session, ora_geometry *geom, char * dest);
static unsigned ewkbPointLen(db2Session *session, ora_geometry *geom);
static char *ewkbPointFill(db2Session *session, ora_geometry *geom, char *dest);
static unsigned ewkbLineLen(db2Session *session, ora_geometry *geom);
static char *ewkbLineFill(db2Session *session, ora_geometry *geom, char * dest);
static unsigned ewkbPolygonLen(db2Session *session, ora_geometry *geom);
static char *ewkbPolygonFill(db2Session *session, ora_geometry *geom, char * dest);
static unsigned ewkbMultiPointLen(db2Session *session, ora_geometry *geom);
static char *ewkbMultiPointFill(db2Session *session, ora_geometry *geom, char *dest);
static unsigned ewkbMultiLineLen(db2Session *session, ora_geometry *geom);
static char *ewkbMultiLineFill(db2Session *session, ora_geometry *geom, char * dest);
static unsigned ewkbMultiPolygonLen(db2Session *session, ora_geometry *geom);
static char *ewkbMultiPolygonFill(db2Session *session, ora_geometry *geom, char * dest);
 ...
 */
static const char *setType(db2Session *session, ora_geometry *geom, const char *data);
static const char *setSridAndFlags(db2Session *session, ora_geometry *geom, const char *data);
static const char *setPoint(db2Session *session, ora_geometry *geom, const char *data);
static const char *setLine(db2Session *session, ora_geometry *geom, const char *data);
static const char *setPolygon(db2Session *session, ora_geometry *geom, const char *data);
static const char *setMultiPoint(db2Session *session, ora_geometry *geom, const char *data);
static const char *setMultiLine(db2Session *session, ora_geometry *geom, const char *data);
static const char *setMultiPolygon(db2Session *session, ora_geometry *geom, const char *data);
...
initSRIDMap()
{
	...
	mapFileName = db2GetShareFileName(SRID_MAP_FILE);

	/* initialize "srid_map" with an invalid entry */
	srid_map = (mapEntry *)malloc(sizeof(mapEntry));
	if (srid_map == NULL)
		db2Error_i(FDW_ERROR, "failed to allocate %d bytes of memory", sizeof(mapEntry));
	srid_map[0].from = 0;
	...
	if ((mapFile = fopen(mapFileName, "r")) == NULL)
	{
		...
		free(srid_map);
		srid_map = NULL;
		db2Error(FDW_ERROR, "cannot open file \"" SRID_MAP_FILE "\": %m");
	}
	...
	do
	{
		...
		if (i == EOF)
		{
			...
			free(srid_map);
			srid_map = NULL;
			db2Error(FDW_ERROR, "syntax error in file \"" SRID_MAP_FILE "\": %m");
		}
		if (i != 2)
		{
			(void)fclose(mapFile);
		...
		if (srid_map == NULL)
		{
			...
			free(srid_map);
			srid_map = NULL;
			db2Error_i(FDW_ERROR, "failed to allocate %d bytes of memory", sizeof(mapEntry) * (count + 1));
		}
	...
	if (errno)
	{
		free(srid_map);
		srid_map = NULL;
		db2Error(FDW_ERROR, "error reading from file \"" SRID_MAP_FILE "\": %m");
	}
}
 ...
 */
ora_geometry *
db2EWKBToGeom(db2Session *session, unsigned ewkb_length, char *ewkb_data)
{
	const char *data = ewkb_data;
	unsigned type;

	ora_geometry *geom = (ora_geometry *)db2Alloc(sizeof(ora_geometry));
	db2GeometryAlloc(session, geom);
	geom->num_elems = -1;
	geom->elem = NULL;
	geom->num_coords = -1;
	geom->coord = NULL;
	...
	switch (type)
	{
		...
		case MULTIPOLYGONTYPE:
			...
			break;
		default:
			db2Error_i(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: unexpected geometry type %u", type);
	}

	/* check that we reached the end of input data */
	if (data - ewkb_data != ewkb_length)
		db2Error_ii(FDW_ERROR, "db2_fdw internal error: number of bytes read %u is different from length %u", data - ewkb_data, ewkb_length);
 ...
 */
unsigned
oracleGetEWKBLen(db2Session *session, ora_geometry *geom)
{
	unsigned type;
	...
	if (geom->indicator->sdo_elem_info == OCI_IND_NOTNULL)
	{
		...
		for (i=0; i<n; i+=3){
			...
			if (!((1 == etype && 1 == interpretation)
				...
				db2Error_ii(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: unsupported etype %u with interpretation %u in elem_info", etype, interpretation);
		}
	}
 ...
 */
char *
oracleFillEWKB(db2Session *session, ora_geometry *geom, unsigned size, char *dest)
{
	...
	if (dest - orig != size)
		db2Error_ii(FDW_ERROR, "db2_fdw internal error: number of bytes written %u is different from size %u", dest - orig, size);
 ...
 */
void
db2GeometryFree(db2Session *session, ora_geometry *geom)
{
	/*
...
/*
 * db2GeometryAlloc
 * 		Allocate memory for a geometry object in the DB2 object cache.
 * 		The indicator is set to atomic NULL.
 */
void
db2GeometryAlloc(db2Session *session, ora_geometry *geom)
{
	...
	if (checkerr(
			OCIObjectNew(session->envp->envhp,
				 		...
				 		session->connp->svchp,
				 		OCI_TYPECODE_OBJECT,
				 		db2GetGeometryType(session),
				 		(dvoid *)NULL,
				 		OCI_DURATION_TRANS,
				 		TRUE,
				 		(dvoid **)&geom->geometry),
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "cannot allocate SDO_GEOMETRY object", oraMessage);
	...
	if (checkerr(
			OCIObjectGetInd(session->envp->envhp,
							...
							(void **)&geom->indicator),
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "cannot get indicator for new SDO_GEOMETRY object", oraMessage);
...
void
appendElemInfo(db2Session *session, ora_geometry *geom, unsigned info)
{
	OCINumber n;
	...
	if (checkerr(
			...
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "cannot append to element info collection", oraMessage);
}

void
appendCoord(db2Session *session, ora_geometry *geom, double coord)
{
	OCINumber n;
	...
	if (checkerr(
			...
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "cannot append to ordinate collection", oraMessage);
}

unsigned
ewkbType(db2Session *session, ora_geometry *geom)
{
	unsigned gtype = 0;
	if (geom->indicator->sdo_gtype == OCI_IND_NULL)
		db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: geometry type cannot be NULL");
	...
	switch (gtype%100)
	{
		...
		case 3:
			return POLYGONTYPE;
		case 4:
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: geometry type COLLECTION not supported");
		case 5:
		...
		case 8:
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: geometry type SOLID not supported");
		case 9:
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: geometry type MULTISOLID not supported");
		default:
			db2Error_i(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: unknown geometry type %u", gtype);
	}
	return 0;  /* unreachable, but keeps compiler happy */
}

const char *
setType(db2Session *session, ora_geometry *geom, const char *data)
{
	...
	switch (wkbType)
	{
		...
		case MULTIPOLYGONTYPE:
			gtype += 7;
			break;
#define UNSUPPORTED_TYPE( T ) case T ## TYPE: db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: geometry type "#T" not supported");
...
#undef UNSUPPORTED_TYPE
		default:
			db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: unknown geometry type %u", wkbType);
	}
 ...
 */
unsigned
ewkbHeaderLen(db2Session *session, ora_geometry *geom)
{
	return 4;
}

char *
ewkbHeaderFill(db2Session *session, ora_geometry *geom, char * dest)
{
	...
	if (sdoDim > 3)
		db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: four-dimensional geometries are unsupported");

	if (ewkbIsMeasured(session, geom))
		/*
		 * In DB2, the measure dimension counts as third dimension,
...
unsigned
ewkbIsMeasured(db2Session *session, ora_geometry *geom)
{
	unsigned gtype = 0;
	...
	switch ((gtype % 1000) / 100)
	{
		...
		case 3:
			return 1;
		default:
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: measure dimension must be \"0\" or \"3\"");
...
/*
 * sdoDimension
 * 		Returns the dimension as DB2 understands it.
 ...
 */
unsigned
sdoDimension(db2Session *session, ora_geometry *geom)
{
	unsigned gtype = 0;
	if (geom->indicator->sdo_gtype == OCI_IND_NOTNULL)
		numberToUint(session->envp->errhp, &(geom->geometry->sdo_gtype), &gtype);
...
unsigned
ewkbSrid(db2Session *session, ora_geometry *geom)
{
	unsigned srid = 0;
	if (geom->indicator->sdo_srid == OCI_IND_NOTNULL)
		numberToUint(session->envp->errhp, &(geom->geometry->sdo_srid), &srid);

	/* convert DB2->PostGIS SRID when needed */
	return epsgFromOracle(srid);
}

const char *
setSridAndFlags(db2Session *session, ora_geometry *geom, const char *data)
{
	unsigned srid = 0;
	unsigned gtype = 0;
	...
	/* convert PostGIS->DB2 SRID when needed */
	...
	if (data[0] & 0x02)
	{
		/*
 		 ...
 		 * is a three-dimensional geometry in DB2, and the measure
 		 * dimension is the third dimension.
 		 */
		gtype += 1300;

		if (gtype >= 4000)
			db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: measure dimension only supported for two-dimensional geometries");
	}
	if (data[0] & 0x08)
		db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: geodetic not supported");
...
unsigned
ewkbPointLen(db2Session *session, ora_geometry *geom)
{
	return 2*sizeof(unsigned) + sizeof(double)*sdoDimension(session, geom);
}

char *
ewkbPointFill(db2Session *session, ora_geometry *geom, char *dest)
{
	...
	if (geom->indicator->sdo_point._atomic != OCI_IND_NOTNULL)
	{
		...
		if (geom->indicator->sdo_elem_info != OCI_IND_NOTNULL)
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: SDO_POINT and SDO_ELEM_INFO cannot both be NULL for a point");
		if (geom->indicator->sdo_ordinates != OCI_IND_NOTNULL)
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: SDO_POINT and SDO_ORDINATES cannot both be NULL for a point");

		if (geom->num_elems < 3)
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: not enough values in SDO_ELEM_INFO");
		...
		if (etype != 1)
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: point cannot have ETYPE different from 1");
		if (interpretation != 1)
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: point with SDO_INTERPRETATION different from 1 is not supported");
	...
	else
	{
		...
		{
			db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: null point coordinates not supported");
		}
...
const char *
setPoint(db2Session *session, ora_geometry *geom, const char *data)
{
	if (*((unsigned *)data) != POINTTYPE)
		db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: expected point, got type %u", *((unsigned *)data));
	data += sizeof(unsigned);
	if (*((unsigned *)data ) != 1)
		db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: empty point is not supported");
	data += sizeof(unsigned);
...
unsigned
ewkbLineLen(db2Session *session, ora_geometry *geom)
{
	return 2 * sizeof(unsigned) + sizeof(double) * geom->num_coords;
}

char *
ewkbLineFill(db2Session *session, ora_geometry *geom, char * dest)
{
	unsigned i;
...
const char *
setLine(db2Session *session, ora_geometry *geom, const char *data)
{
	unsigned i, n;

	if (*((unsigned *)data) != LINETYPE)
		db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: expected line, got type %u", *((unsigned *)data));
	data += sizeof(unsigned);
	n = *((unsigned *)data) * sdoDimension(session, geom);
	data += sizeof(unsigned);

	if (!n)
		db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: empty line is not supported");
...
unsigned
ewkbPolygonLen(db2Session *session, ora_geometry *geom)
{
...
const char *
setPolygon(db2Session *session, ora_geometry *geom, const char *data)
{
	...
	if (*((unsigned *)data) != POLYGONTYPE)
		db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: expected polygon, got type %u", *((unsigned *)data));
	data += sizeof(unsigned);
	...
	if (!numRings)
		db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: empty polygon is not supported");
...
char *
ewkbPolygonFill(db2Session *session, ora_geometry *geom, char * dest)
{
	const unsigned dimension = sdoDimension(session, geom);
	const unsigned numRings = geom->num_elems / 3;
	const unsigned numC = geom->num_coords;
...
unsigned
ewkbMultiPointLen(db2Session *session, ora_geometry *geom)
{
	const unsigned numC = geom->num_coords;
	const unsigned numPoints = numC / sdoDimension(session, geom);
...
char *
ewkbMultiPointFill(db2Session *session, ora_geometry *geom, char * dest)
{
	unsigned i;
...
const char *
setMultiPoint(db2Session *session, ora_geometry *geom, const char *data)
{
	unsigned i, j, numPoints;
	const unsigned dimension = sdoDimension(session, geom);

	if (*((unsigned *)data) != MULTIPOINTTYPE)
		db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: expected multipoint, got type %u", *((unsigned *)data));
	data += sizeof(unsigned);
	numPoints = *((unsigned *)data);
	data += sizeof(unsigned);

	if (!numPoints)
		db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: empty multipoint is not supported");

	for (i=0; i<numPoints; i++)
	{
		if (*((unsigned *)data) != POINTTYPE)
			db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: expected point in multipoint, got type %u", *((unsigned *)data));
		data += sizeof(unsigned);
		if (*((unsigned *)data ) != 1)
			db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: empty point in multipoint is not supported");
		data += sizeof(unsigned);
...
unsigned
ewkbMultiLineLen(db2Session *session, ora_geometry *geom)
{
...
char *
ewkbMultiLineFill(db2Session *session, ora_geometry *geom, char * dest)
{
	unsigned i;
	const unsigned numC = geom->num_coords;
	const unsigned dimension = sdoDimension(session, geom);
...
const char *
setMultiLine(db2Session *session, ora_geometry *geom, const char *data)
{
	unsigned r, numLines;
	if (*((unsigned *)data) != MULTILINETYPE)
		db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: expected multiline, got type %u", *((unsigned *)data));
	data += sizeof(unsigned);
	numLines = *((unsigned *)data);
	data += sizeof(unsigned);

	if (!numLines)
		db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: empty multiline is not supported");
...
unsigned
ewkbMultiPolygonLen(db2Session *session, ora_geometry *geom)
{
...
char *
ewkbMultiPolygonFill(db2Session *session, ora_geometry *geom, char * dest)
{
	const unsigned dimension = sdoDimension(session, geom);
	const unsigned numC = geom->num_coords;
	const unsigned totalNumRings = geom->num_elems / 3;
...
const char *
setMultiPolygon(db2Session *session, ora_geometry *geom, const char *data)
{
	unsigned p, numPolygons;

	if (*((unsigned *)data) != MULTIPOLYGONTYPE)
		db2Error_i(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: expected multipolygon, got type %u", *((unsigned *)data));
	data += sizeof(unsigned);
	numPolygons = *((unsigned *)data);
	data += sizeof(unsigned);

	if (!numPolygons)
		db2Error(FDW_ERROR, "error converting geometry to SDO_GEOMETRY: empty multipolygon is not supported");
 ...
 */
unsigned
numCoord(db2Session *session, ora_geometry *geom)
{
	int n;
	...
	if (checkerr(
			OCICollSize(session->envp->envhp,
						...
						&n),
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "cannot get size of ordinate collection", oraMessage);
	return n;
}
 ...
 */
double
coord(db2Session *session, ora_geometry *geom, unsigned i)
{
	double coord;
	boolean exists;
	...
	if (checkerr(
			OCICollGetElem(session->envp->envhp, session->envp->errhp,
				   		...
				   		(dvoid **)&indicator),
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "error fetching element from ordinate collection", oraMessage);
	if (! exists)
		db2Error_i(FDW_ERROR, "element %u of ordinate collection does not exist", i);
	if (*indicator == OCI_IND_NULL)
		db2Error_i(FDW_ERROR, "element %u of ordinate collection is NULL", i);
 ...
 */
unsigned
numElemInfo(db2Session *session, ora_geometry *geom)
{
	int n;
	if (checkerr(
			OCICollSize (session->envp->envhp,
				 		...
				 		&n),
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "cannot get size of element info collection", oraMessage);
	return n;
}
 ...
 */
unsigned
elemInfo(db2Session *session, ora_geometry *geom, unsigned i)
{
	unsigned info;
	boolean exists;
	OCINumber *oci_number;
	...
	if (checkerr(
			OCICollGetElem(session->envp->envhp, session->envp->errhp,
				   		...
				   		(dvoid **)&indicator),
			session->envp->errhp) != OCI_SUCCESS)
		db2Error_d(FDW_ERROR, "error fetching element from element info collection", oraMessage);
	if (! exists)
		db2Error_i(FDW_ERROR, "element %u of element info collection does not exist", i);
	if (*indicator == OCI_IND_NULL)
		db2Error_i(FDW_ERROR, "element %u of element info collection is NULL", i);
 ...
 */
void
unpack(db2Session *session, ora_geometry *geom)
{
	...
	if (geom->indicator->sdo_elem_info != OCI_IND_NOTNULL
			|| geom->indicator->sdo_ordinates != OCI_IND_NOTNULL)
		db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: SDO_ELEM_INFO and SDO_ORDINATES must either both be NULL");

	elemCount = numElemInfo(session,geom);
	coordCount = numCoord(session,geom);

	if (elemCount%3 != 0)
		db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: size of SDO_ELEM_INFO must be a multiple of three");

	/* this might be too big if some elements with etype 0 are deleted */
	geom->elem = db2Alloc(elemCount * sizeof(unsigned));
	geom->coord = db2Alloc(coordCount * sizeof(double));
	...
	if (elem_pos == 0)
		db2Error(FDW_ERROR, "error converting SDO_GEOMETRY to geometry: SDO_ELEM_INFO must contain element with non-zero ETYPE");
}
 ...
 */
void
freeUnpacked(db2Session *session, ora_geometry *geom)
{
	if (geom->elem != NULL)
		db2Free(geom->elem);
	if (geom->coord != NULL)
